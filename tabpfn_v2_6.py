...
class TabPFNV2p6(Architecture):
    ...
    def forward(
        ...
        *,
        ...
        # >>> STREAM_SOFT_PROMPT_PATCH_V26
        # Optional embedding-level prompt injection.
        #
        # This does not add extra rows/tokens and does not change single_eval_pos.
        # It only shifts the already-created x/y embeddings before concatenation.
        #
        # Accepted prompt shapes:
        #   prompt["x"]: [D] or broadcastable to embedded_x_BRiGX [B, R, G, D]
        #   prompt["y"]: [D] or broadcastable to embedded_y_BRiX [B, R, D]
        #
        # If `stream_prompt` is not passed through the forward call, the model also
        # checks `self._tabpfn_stream_prompt`, which is convenient when using the
        # sklearn-style TabPFNClassifier API.
        if stream_prompt is None:
            stream_prompt = getattr(self, "_tabpfn_stream_prompt", None)

        if stream_prompt is not None:
            prompt_mode = stream_prompt.get("mode", "both")
            prompt_scale = float(stream_prompt.get("scale", 1.0))

            if prompt_mode in ("both", "x_only") and stream_prompt.get("x", None) is not None:
                prompt_x = stream_prompt["x"].to(
                    device=embedded_x_BRiGX.device,
                    dtype=embedded_x_BRiGX.dtype,
                )
                if prompt_x.ndim == 1:
                    prompt_x = prompt_x.view(1, 1, 1, -1)
                elif prompt_x.ndim == 2:
                    # Usually [B, D] or [R, D]; user must ensure broadcastability.
                    prompt_x = prompt_x[:, None, None, :]
                while prompt_x.ndim < embedded_x_BRiGX.ndim:
                    prompt_x = prompt_x.unsqueeze(0)
                embedded_x_BRiGX = embedded_x_BRiGX + prompt_scale * prompt_x

            if prompt_mode in ("both", "y_only") and stream_prompt.get("y", None) is not None:
                prompt_y = stream_prompt["y"].to(
                    device=embedded_y_BRiX.device,
                    dtype=embedded_y_BRiX.dtype,
                )
                if prompt_y.ndim == 1:
                    prompt_y = prompt_y.view(1, 1, -1)
                elif prompt_y.ndim == 2:
                    prompt_y = prompt_y[:, None, :]
                while prompt_y.ndim < embedded_y_BRiX.ndim:
                    prompt_y = prompt_y.unsqueeze(0)
                embedded_y_BRiX = embedded_y_BRiX + prompt_scale * prompt_y
        # <<< STREAM_SOFT_PROMPT_PATCH_V26
